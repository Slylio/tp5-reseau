/*
 * CASA
 * Copyright (c) 2020 IRISA Laboratory, University of South Brittany, France.
 * All rights reserved.
 *
 * File    : $DataBufferizer.java$ 
 * Author  : $F. Guidec, P. Launay$
 * Version : $1.2$
 * Date    : $Fri Dec 18 17:36:24 CET 2020$
 */
...
public class DataBufferizer
{
    ...
    {
	int val = (buffer[offset] & 0xFF) << 8;
	val |= (buffer[offset+1] & 0xFF);
	return val;
    }

    //------------------------------------------------------------
    public static void writeUnsignedShort(int v, byte[] buffer, int offset)
    {
	buffer[offset] = (byte)(0xFF & (v >> 8));
	buffer[offset+1] = (byte)(0xFF & v);
    }
    ...
    //------------------------------------------------------------
    public static String bufferToString(byte[] buffer, int offset, int length)
    {
    ...
    //------------------------------------------------------------
    public static String bufferToBinaryString(byte[] buffer, int offset, int length)
    {
	StringBuffer result = new StringBuffer();
	int max = (length+offset>buffer.length?buffer.length:length+offset);
	for (int i=offset; i<max; i++) {
    ...
    //------------------------------------------------------------
    public static boolean compareBuffer(byte[] sourceBuffer, int sourceOffset, byte[] toBuffer, int toOffset, int length)
    {
